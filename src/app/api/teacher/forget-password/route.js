...
export const POST = async (request) => {
    ...
    try {
        ...
        if (!password && email) {
            const verifyEmail = await teacherModel.findOne({ email: email });
            if (!verifyEmail) {
        ...
        const isEmail = await teacherModel.findOne({ email: email });
        if (!isEmail) {
            return NextResponse.json({ message: "Unable to update password", status: 0 })
        }

        const hashedPassword = await bcrypt.hash(password, 10)
        const updatePassword = await teacherModel.findByIdAndUpdate({ _id: isEmail._id }, { password: hashedPassword });
        if (!updatePassword) {
            return NextResponse.json({ message: "Unable to update password", status: 0 })
        }
